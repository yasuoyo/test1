package com.example.test.upload.controller;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.core.util.StrUtil;
import com.example.test.upload.DeleteQO;
import com.example.test.upload.api.ISysBaseAPI;
import com.example.test.upload.utils.CommonUtils;
import com.example.test.upload.utils.OssConstants;
import com.example.test.upload.utils.R;
...
//		String bizPath = request.getParameter("path");
		...
		if(StrUtil.isEmpty(bizPath)){
			if(OssConstants.UPLOAD_TYPE_OSS.equals(uploadType)){
		...
		if(OssConstants.UPLOAD_TYPE_LOCAL.equals(uploadType)){
			//针对jeditor编辑器如何使 lcaol模式，采用 base64格式存储
			String jeditor = request.getParameter("jeditor");
			if(StrUtil.isNotEmpty(jeditor)){
				result.setMsg(OssConstants.UPLOAD_TYPE_LOCAL);
				result.setData(OssConstants.UPLOAD_TYPE_LOCAL);
				result.setCode(0);
				return result;
			}else{
