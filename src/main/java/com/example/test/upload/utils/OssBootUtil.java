...
//	@Value(value = "${upload.oss.staticDomain}")
    ...
    public static String upload(MultipartFile file, String fileDir, String customBucket) {
        ...
        try {
			...
			if (fileDir.length() > 100) {
				fileDir = fileDir.substring(0, 100);
			}
            ...
            if (StrUtil.isNotEmpty(staticDomain) && staticDomain.toLowerCase().startsWith("http")) {
                ...
                FILE_URL = OssConstants.UPLOAD_OSS_HTTPS + newBucket + "." + endPoint + "/" + fileUrl;
            }
            PutObjectResult result = ossClient.putObject(newBucket, fileUrl.toString(), file.getInputStream());
            // 设置权限(公开读)
//            ossClient.setBucketAcl(newBucket, CannedAccessControlList.PublicRead);
            if (result != null) {
                log.info(OssConstants.UPLOAD_OSS_SUCCESS + fileUrl);
            }
        } catch (IOException e) {
    ...
    public static String upload(FileItemStream file, String fileDir) {
        ...
        try {
            ...
            if (StrUtil.isNotEmpty(staticDomain) && staticDomain.toLowerCase().startsWith("http")) {
                ...
                FILE_URL = OssConstants.UPLOAD_OSS_HTTPS + bucketName + "." + endPoint + "/" + fileUrl;
            }
    ...
    public static void deleteUrl(String url,String bucket) {
        ...
        if (StrUtil.isNotEmpty(staticDomain) && staticDomain.toLowerCase().startsWith("http")) {
            ...
            bucketUrl = OssConstants.UPLOAD_OSS_HTTPS + newBucket + "." + endPoint + "/";
        }
