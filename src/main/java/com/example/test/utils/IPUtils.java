...
/*            //todo 测试桩
            ...
            if (StrUtil.isEmpty(ip) || IP_STATUS_UNKNOWN.equalsIgnoreCase(ip)) {
                ip = request.getHeader(HTTP_HEADER_PROXYCLIENTIP);
            }
            if (StrUtil.isEmpty(ip) || ip.length() == 0 || IP_STATUS_UNKNOWN.equalsIgnoreCase(ip)) {
...
//        logger.info("ip="+ip);
        //使用代理，则获取第一个IP地址
        if (StrUtil.isNotEmpty(ip) && ip.indexOf(",") >= 0) {
